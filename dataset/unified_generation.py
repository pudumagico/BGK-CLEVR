...
#         # print(x_center, cx*PIXEL_SCALE)
#         # print(y_center, cy*PIXEL_SCALE)

#         labels.append(f'{class_index} {cx/(grid_size)} {cy/(grid_size)} {x_width/(grid_size*PIXEL_SCALE)} {y_width/(grid_size*PIXEL_SCALE)}\n')
...
def generate_model(grid_size, constraints, constraint_flags, existing_models):
    return
...
for i, _ in enumerate(tqdm(range(10))):
    ...
    labels, random_move = gen_image(i, model_str.split(' '), grid_size, './test_dataset/1', True)
...
for i, _ in enumerate(tqdm(range(10))):
    ...
    labels, random_move = gen_image(i, model_str.split(' '), grid_size, './test_dataset/0', True)
